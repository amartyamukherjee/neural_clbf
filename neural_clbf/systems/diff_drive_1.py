...
class DiffDriveProblem1(ControlAffineSystem):
    """
    """
    ...
    @property
    def n_dims(self) -> int:
        return DiffDriveProblem1.N_DIMS

    @property
    def angle_dims(self) -> List[int]:
        return [DiffDriveProblem1.THETA]

    @property
    def n_controls(self) -> int:
        return DiffDriveProblem1.N_CONTROLS
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[DiffDriveProblem1.X] = 1.0
        upper_limit[DiffDriveProblem1.Y] = 1.0
        upper_limit[DiffDriveProblem1.THETA] = 10000.0
    ...
    def _g(self, x: torch.Tensor, params: Scenario):
        ...
        theta = x[:, DiffDriveProblem1.THETA]

        g[:,DiffDriveProblem1.X,DiffDriveProblem1.UL] = torch.cos(theta)*r/2
        g[:,DiffDriveProblem1.X,DiffDriveProblem1.UR] = torch.cos(theta)*r/2
        g[:,DiffDriveProblem1.Y,DiffDriveProblem1.UL] = torch.sin(theta)*r/2
        g[:,DiffDriveProblem1.Y,DiffDriveProblem1.UR] = torch.sin(theta)*r/2
        g[:,DiffDriveProblem1.THETA,DiffDriveProblem1.UL] = -r/(2*d)
        g[:,DiffDriveProblem1.THETA,DiffDriveProblem1.UR] = r/(2*d)
